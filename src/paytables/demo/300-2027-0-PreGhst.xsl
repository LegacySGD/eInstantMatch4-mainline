			...
			<lxslt:component prefix="my-ext" functions="formatJson">
				<lxslt:script lang="javascript">
					...
					function formatJson(jsonContext, translations, prizeTable, prizeValues, prizeNames)
					{
						...
						var matches = scenario.split(',');											
						var convertedPrizeValues = (prizeValues.substring(1)).split('|');
						var matchNames = ["first","second","third","fourth"];
						...
						r.push(getTranslationByName("position", translations));
						r.push('</td>');
						...
						for(var i = 0; i < matches.length; ++i)
						{
							...
							r.push(getTranslationByName(matchNames[i], translations));
							r.push('</td>');
